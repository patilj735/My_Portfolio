      ...
      {project && (
        ...
        >
          <motion.div
            ...
            className="glass relative max-h-[86vh] w-full max-w-3xl overflow-y-auto rounded-2xl border border-base-border shadow-glow"
          >
            <button
              onClick={onClose}
              aria-label="Close project details"
              className="absolute right-4 top-4 z-10 flex h-9 w-9 items-center justify-center rounded-full border border-base-border bg-base-bg/70 text-ink-secondary backdrop-blur-md transition-colors hover:text-ink-primary"
            >
              <X size={16} />
            </button>

            <div className="relative aspect-[16/8] w-full overflow-hidden border-b border-base-border bg-gradient-to-br from-base-raised via-base-surface to-base-bg">
              {project.image ? (
                <img
                  src={project.image}
                ...
                <>
                  ...
                  <div className="flex h-full items-center justify-center">
                    <Layers size={40} className="text-ink-muted" />
                  </div>
                </>
              )}

              {project.image && (
                <div className="absolute inset-0 bg-gradient-to-t from-base-bg/30 via-transparent to-transparent" />
              )}
            </div>
              ...
              {project.metrics && (
                <div className="mt-6 grid grid-cols-2 gap-3">
                  {project.metrics.map((m) => (
                    <div
                      key={m.label}
                      className="rounded-xl border border-base-border bg-base-raised/60 p-4"
                    >
